      ...
      // }
	   $job_types = DB::table('job_types')->get();
        ...
        if (!empty($request->search_term)) {
            $userTypes = DB::table('user_types')->select(array('id'))->where('user_type', 'like', '%' . $request->search_term . '%')->get();
            $data1 = collect($userTypes)->map(function($x) {
                        return (array) $x;
                    })->toArray();
            if (count($data1) > 0) {
                $position[] = $data1[0]['id'];
            }
            //Get job positions id based on search keyword
            $job_types = DB::table('job_types')->select(array('id'))->where('name', 'like', '%' . $request->search_term . '%')->get();
            $data2 = collect($job_types)->map(function($x) {
                        return (array) $x;
                    })->toArray();
            ...
            $skills = DB::table('skills')->select('id')->where('skill', 'like', '%' . $request->search_term . '%')->get();
            $data3 = collect($skills)->map(function($x) {
                        return (array) $x;
                    })->toArray();

            if (count($data3) > 0) {
                foreach ($data3 as $skill) {
        ...
        $job_type = $request->job_type;
         ...
         $results = DB::table('jobs')
                ...
                ->where(function ($dbquery) use ($search_term, $skillsArray) {
                    $dbquery->when($search_term, function($query) use ($search_term, $skillsArray) {
                        $query->orWhere('name', 'LIKE', "%" . $search_term . "%");
                        //commented by harpreet
                        //$query->orWhere('description', 'LIKE', "%" . $search_term . "%");
                        //$query->orWhere('office_information.city', 'LIKE', "%" . $search_term . "%");
                        //$query->orWhere('office_information.state', 'LIKE', "%" . $search_term . "%");
                        //$query->orWhere('office_information.zipcode', 'LIKE', "%" . $search_term . "%");
                        $query->orWhere(function ($dbquery) use ($skillsArray) {
                            $dbquery->when($skillsArray, function($sql) use ($skillsArray) {
                                return $sql->whereIn('skill_id', $skillsArray);
                            });
                        });
                        return $query;
                    });
