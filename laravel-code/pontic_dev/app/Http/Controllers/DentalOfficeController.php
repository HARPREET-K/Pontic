       ...
       $jobseekers = DB::table('users')
        ...
        ->whereRaw('travel_distance >= ROUND(( 3956 * acos( cos( radians(?) )
                   ...
                   * sin( radians( latitude ) ) )))',
                   [$usersss[0]->latitude, $usersss[0]->longitude, $usersss[0]->latitude])
...
foreach ($jobseekers as $var)
    ...
    $implodeId =  implode(' ', $jobseekersId);
    $explodeId =  explode(' ', $implodeId);
  $arrayId = array();
  foreach ($explodeId as $var)
      {
          $var;
         
        $mm  = DB::table('user_avalibility_dates')
        ->where('user_id','=',$var)->get();
         
        // echo $mm = "select * from user_avalibility_dates where user_id = '$kachra'";
        // $resultss = mysql_query($mm);
        $startTime = array();    
        foreach($mm as $print)
             {
                   $startTime[] = $print->start_time;
            }
            $arrayId[] = implode("\n", $startTime);
           
       }
         $user_avalibility_dates2 = implode("\n\n", $startTime);
  ...
  //  die();
        ...
        $messageCounter = DB::table('messages')->where('is_seen', '=', 0)->where('reciever_id', $user->id)->get();
        return view('do-js-search', compact('data','messageCounter','user_avalibility_dates2', 'user', 'haveaddress', 'credentials','available_jobseekers', 'languages','available', 'certifications','usersss', 'sofaware_skillss','radiology_skills', 'jobseekers', 'officeInformation', 'officeImages'));
    }
    ...
    public function getJsList2(Request $request) {
       ...
       $qwerty1 = implode(" ",$var); 
       //$query->orWhere('credential','LIKE', '%BS%')->orWhere('credential','LIKE', '%RDH%');
  $qwerty = implode("_orWhere('credential','LIKE', '%",$var);   
  $qwerty = str_replace("_","%')->",$qwerty);
  $qwerty = "orWhere('credential','LIKE', '%".$qwerty."%')";

  $credential = explode( " ",$qwerty1);
...
//$thisrt = explode(',', $request->experience);
       // foreach ($thisrt as $key => $value) {
            // $mm[] = $value;
                ...
                ->where(function ($dbquery) use ($location) {
                 ...
                 ->where(function ($dbquery) use ($credential) {
                    $dbquery->when($credential, function($query) use ($credential) {
                        ...
                        {
               return $query->orWhere('credential','LIKE', "%$val%");  
                        }
//                        return $query->orWhere('credential','LIKE', '%BS%')->orWhere('credential','LIKE', '%RDH%');
                //  return $query->$qwerty;
                    });
                })
                ...
                ->where(function ($dbquery) use ($language) {
                   ...
                   return $query->whereIn('languages', $language);
                });
                })
        ...
        $sofaware_skillss = DB::table('skills')->where('skill_type', '=', 1)->orderBy('skill', 'ASC')->get();
        $radiology_skills = DB::table('skills')->where('skill_type', '=', 2)->orderBy('skill', 'ASC')->get();
        if($request->search_term !='') { $available_jobseekers = ''; }
        else {
        ...
        if($request->search_term !=''){ $available =''; }
        else {
        $available = 'Other Available Job Seekers';
        }
        
        $messageCounter = DB::table('messages')->where('is_seen', '=', 0)->where('reciever_id', $user->id)->get();
        return view('do-js-search', compact('data','messageCounter', 'credentials', 'languages', 'certifications','available_jobseekers','available', 'user','usersss','sofaware_skillss','radiology_skills', 'jobseekers', 'officeInformation', 'officeImages'));
        }
...
$messageCounter = DB::table('messages')->where('is_seen', '=', 0)->where('reciever_id', $user->id)->get();
        return view('do-js-profile', compact('data','messsageCounter', 'user', 'jobseeker', 'officeInformation', 'officeImages', 'sofaware_skills', 'radiology_skills', 's_skillIDS', 'user_skills_software', 'r_skillIDS', 'user_skills_radiology', 'user_education', 'user_exprience', 'user_certifications', 'available_timings'));
    }
